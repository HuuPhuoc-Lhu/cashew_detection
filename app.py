...
# ---------- Upload model nếu chưa có ----------
if not os.path.exists("best.pt"):
    uploaded_model = st.file_uploader("📤 Tải lên mô hình YOLOv8 (.pt)", type=["pt"])
    if uploaded_model is not None:
        with open("best.pt", "wb") as f:
            f.write(uploaded_model.getbuffer())
        st.success("✅ Mô hình đã được lưu thành công!")
...
def load_model(path="best.pt"):
    try:
        model = YOLO(path)
        st.write("Model names:", model.names)
        return model
...
if uploaded_file is not None and detect_model is not None:
    ...
    st.info("⬆️ Hãy tải lên 1 ảnh và đảm bảo mô hình đã được load để bắt đầu dự đoán.")
